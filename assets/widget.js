...
(() => {
  ...
  function renderMap() {
    ...
    if (fresh) {
      ...
      map.on('moveend', () => {
        ...
        mapMoveTimer = setTimeout(() => {
          ...
          if (root) refreshList(root); // update list only, no scroll
        }, 300);
      });
      ...
      map.on('popupopen', () => {
        ...
        btn.addEventListener('click', () => {
          ...
          map.closePopup();
          // Ensure the location is in the ranked list by setting searchPoint to it
    ...
    allActive.forEach(loc => {
      ...
      // Click only opens popup — scrolling happens only when user presses "Visa tider i listan"
