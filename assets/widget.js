(() => {
  ...
  function rankLocations(forceAll = false) {
    ...
    return { ranked: pool, showingAll: !!center && forceAll };
  }
  ...
  function locationCard(loc) {
    ...
    return `<article class="mw-card${expanded ? ' is-expanded' : ''}" data-location-id="${esc(loc.id)}"><button class="mw-card-toggle" type="button" data-toggle-card="${esc(loc.id)}" aria-expanded="${expanded ? 'true' : 'false'}"><div class="mw-card-top"><div class="mw-card-title-wrap"><h3 class="mw-card-title">${esc(loc.name)}</h3>${addressHtml}<div class="mw-card-meta">${distHtml}<div class="mw-badges">${svcBadges}</div></div></div><div class="mw-card-right">${nextHtml}<div class="mw-accordion-cta">${expanded ? 'Dölj tider' : 'Visa fler tider'}<span class="mw-accordion-count">${esc(countText)}</span></div></div></div></button><div class="mw-card-details"${expanded ? '' : ' hidden'}><div class="mw-card-details-inner"><div class="mw-upcoming-block"><div class="mw-upcoming-title">Kommande tider</div><ul class="mw-upcoming-list">${detailRows}</ul></div>${buildActionButtons(loc)}</div></div></article>`;
  }
    ...
    if (fresh) {
      ...
      map.on('popupopen', (evt) => {
        ...
        btn.addEventListener('click', () => {
          ...
          state.expandedId = id;
          refreshList(root, { preserveMapViewport: false });
    ...
    if (!state.mapCenter) {
      if (state.searchPoint) {
        ...
        requestAnimationFrame(() => {
          map.invalidateSize();
          fitMapToAllActive();
          setTimeout(() => { map.invalidateSize(); fitMapToAllActive(); }, 150);
        });
      }
    }
  }
  ...
  function refreshList(root, opts = {}) {
    ...
    if (opts.preserveMapViewport) preserveMapViewport(root, doUpdate); else doUpdate();
  }
